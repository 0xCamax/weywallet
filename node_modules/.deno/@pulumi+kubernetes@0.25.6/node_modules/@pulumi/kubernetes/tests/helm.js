...
const helm = require("../helm/index");
const helmSort = helm.v2.helmSort;
function makeKinds(kinds) {
...
describe("helmSort", () => {
    ...
    it("correctly sorts duplicates", () => {
        assert.deepStrictEqual(makeKinds(["Namespace", "LimitRange", "Namespace"]).sort(helmSort), makeKinds(["Namespace", "Namespace", "LimitRange"]));
    });
    it("puts unknown kinds last", () => {
        assert.deepStrictEqual(makeKinds(["UNKNOWN KIND", "LimitRange", "Namespace"]).sort(helmSort), makeKinds(["Namespace", "LimitRange", "UNKNOWN KIND"]));
    });
    it("sorts a shuffled array", () => {
        const shuffled = makeKinds(shuffle([
            "Namespace",
            "ResourceQuota",
            "LimitRange",
            "PodSecurityPolicy",
